...
const EditSpecies = () => {
  ...
  useEffect(() => {
    const fetchSpecies = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/species/${id}`);
  ...
  return (
    <div className="min-h-screen px-6 py-10">
      ...
      >
        <div className="mb-5">
          <label className="block text-left text-sm text-gray-300 mb-1">Species Name</label>
          <input
            type="text"
            ...
            <img
              src={
                currentImage.startsWith('/uploads')
                  ? `${import.meta.env.VITE_API_BASE_URL}${currentImage}`
                  : currentImage
              }
              alt="Current"
        ...
        <button
          type="submit"
          className="w-full py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white font-semibold shadow"
        >
          Update Species
        </button>
      </form>
