...
const AddWorld = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || 'Failed to add world');
      }
  ...
  return (
    <div className="min-h-screen px-6 py-10">
      <h2 className="text-3xl font-bold text-center mb-8 text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-fuchsia-400">
        Add New World
      </h2>
      ...
      >
        <div className="mb-5">
          <label className="block text-left text-sm text-gray-300 mb-1">World Name</label>
          <input
            type="text"
        ...
        <div className="mb-6">
          <label className="block text-left text-sm text-gray-300 mb-1">World Image</label>
          <input
            type="file"
            accept="image/*"
        ...
        <button
          type="submit"
          className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold shadow"
        >
          Add World
        </button>
      </form>
